...
function processOutcome( expectedState, handlerResult ) {

    const { state, err, result } = handlerResult;

    if( state !== expectedState ) {

        throw new FailError( `${state.method} called instead of ${expectedState.method}`,
                             err, result );
    }
...
class LambdaRunner {
    ...
    async run( handler ) {
        ...
        try {
            ...
            let result = processOutcome( this.expectedState, handlerResult );
